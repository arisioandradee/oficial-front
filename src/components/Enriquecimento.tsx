...
const mockHistory = [
  { id: '20246570', name: 'Lista #20246570', date: '12/03/2026', leads: 1, status: 'Concluído', progress: 100, createdBy: 'Super Admin', type: 'excel' },
  { id: 'fe4820ca', name: 'Dibai', date: '12/03/2026', leads: 1, status: 'Concluído', progress: 100, createdBy: 'Super Admin', type: 'manual' },
  { id: 'e8522db1', name: 'teste2', date: '10/03/2026', leads: 1, status: 'Concluído', progress: 100, createdBy: 'Super Admin', type: 'excel' },
  { id: 'a6b75ad0', name: 'teste', date: '10/03/2026', leads: 1, status: 'Concluído', progress: 100, createdBy: 'Super Admin', type: 'manual' },
  { id: '493d9887', name: 'Enriquecimento 09/03/2026', date: '09/03/2026', leads: 1, status: 'Concluído', progress: 100, createdBy: 'userteste1', type: 'excel' },
];
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setView('form')} className="p-2 hover:bg-brand-hover rounded-full transition-colors">
              <ChevronLeft className="w-5 h-5 text-brand-text-dim" />
            </button>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="relative flex-1 w-full">
                <FileText className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-text-dim" />
                <input 
                  type="text"
                  placeholder="Buscar por nome da lista..."
                  className="w-full pl-11 pr-4 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-medium text-brand-text outline-none focus:border-brand-primary placeholder:text-brand-text-dim transition-all"
                />
              </div>
              
              <div className="relative group">
                <button className="flex items-center gap-2 px-6 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-black uppercase tracking-widest text-brand-text-dim hover:text-brand-primary hover:border-brand-primary transition-all whitespace-nowrap min-w-[180px] justify-between">
                  <span className="flex items-center gap-2">
                    <ChevronDown className="w-4 h-4" />
                    Todos os status
                  </span>
                </button>
                <div className="absolute right-0 top-full mt-2 w-48 bg-brand-card border border-brand-border rounded-xl shadow-2xl z-50 py-1 hidden group-hover:block">
                  {['Todos os status', 'Pendente', 'Processando', 'Concluído', 'Falhou'].map((status) => (
                    <button 
                      key={status}
                      className="w-full px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-brand-text-dim hover:text-brand-primary hover:bg-brand-bg/50 transition-all text-left"
                    >
                      {status}
                    </button>
                  ))}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-brand-border bg-brand-bg/50">
                    ...
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest text-center">Status</th>
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest text-center">Leads</th>
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest text-center">Progresso</th>
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest">Criada por</th>
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest">Criada em</th>
                    <th className="p-4 text-[10px] font-black text-brand-text-dim uppercase tracking-widest text-right">Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {mockHistory.map((item) => (
                    <tr key={item.id} className="border-b border-brand-border hover:bg-brand-hover/50 transition-colors group">
                      <td className="p-4">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-xs font-bold text-brand-text group-hover:text-brand-primary transition-colors">{item.name}</span>
                          <span className="text-[9px] font-medium text-brand-text-dim">#{item.id}</span>
                        </div>
                      </td>
                      <td className="p-4 text-center">
                        <span className={cn(
                          "px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest border mx-auto inline-block",
                          item.status === 'Concluído' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-fuchsia-500/10 text-fuchsia-500 border-fuchsia-500/20"
                        )}>
                          {item.status}
                        </span>
                      </td>
                      <td className="p-4 text-center text-xs font-bold text-brand-text">{item.leads}</td>
                      <td className="p-4 text-center text-xs font-bold text-brand-text">{item.progress}%</td>
                      <td className="p-4 text-xs font-medium text-brand-text-dim">{item.createdBy}</td>
                      <td className="p-4 text-xs font-medium text-brand-text-dim">{item.date}</td>
                      <td className="p-4 text-right">
                        <button 
                          ...
                          className="px-4 py-1.5 bg-brand-bg border border-brand-border text-brand-text rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-brand-primary hover:text-white hover:border-brand-primary transition-all active:scale-95"
                        >
                          Ver
                        </button>
                      </td>
