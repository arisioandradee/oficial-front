        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div className="py-2">
            <FilterItem 
              ...
              icon={Search} 
              active={activeFilter === 'opcoes'} 
              onClick={() => setActiveFilter(activeFilter === 'opcoes' ? null : 'opcoes')}
            >
            ...
            <FilterItem 
              ...
              icon={Building2} 
              active={activeFilter === 'segmento'} 
              onClick={() => setActiveFilter(activeFilter === 'segmento' ? null : 'segmento')}
            >
            ...
            <FilterItem 
              ...
              icon={Code} 
              active={activeFilter === 'atividade'} 
              onClick={() => setActiveFilter(activeFilter === 'atividade' ? null : 'atividade')}
            >
              <div className="relative mt-2">
                <div className="relative">
                  <input 
            ...
            <FilterItem 
              title="Localização" 
              icon={MapPin} 
              active={activeFilter === 'localizacao'} 
              onClick={() => setActiveFilter(activeFilter === 'localizacao' ? null : 'localizacao')}
            >
              <div className="space-y-3 mt-2">
                <div className="relative">
                  <button 
                    ...
                    className="w-full flex items-center justify-between px-4 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-medium text-brand-text-dim hover:border-brand-primary transition-all"
                  >
                    <span>Estados e Distrito Federal</span>
                      ...
                      >
                        ...
                        <div className="p-3 bg-brand-bg/50 border-t border-brand-border">
                          <button 
                            ...
                            className="w-full py-2 text-[9px] font-black text-brand-primary uppercase tracking-widest border border-brand-primary/20 rounded-lg hover:bg-brand-primary/5 transition-all text-left px-3"
                          >
                            Selecionar todos ({states.length})
                          </button>
                        </div>
                      </motion.div>
                    )}
                ...
                <button className="w-full flex items-center justify-between px-4 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-medium text-brand-text-dim hover:border-brand-primary transition-all">
                  <span>Selecione uma ou mais cidades</span>
                  <ChevronDown className="w-3 h-3" />
                </button>

                <button className="w-full flex items-center justify-between px-4 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-medium text-brand-text-dim hover:border-brand-primary transition-all">
                  <span>Selecione um ou mais bairros</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
                ...
                <button className="w-full flex items-center justify-between px-4 py-3 bg-brand-bg border border-brand-border rounded-xl text-[11px] font-medium text-brand-text-dim hover:border-brand-primary transition-all">
                  <span>Pesquise por endereço</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
              </div>
            </FilterItem>
            ...
            */}
            <FilterItem 
              title="Faturamento" 
              icon={DollarSign} 
              active={activeFilter === 'faturamento'} 
              onClick={() => setActiveFilter(activeFilter === 'faturamento' ? null : 'faturamento')}
            >
              <div className="relative mt-2">
                <button 
            ...
            <FilterItem 
              title="Porte" 
              icon={TrendingUp} 
              active={activeFilter === 'porte'} 
              onClick={() => setActiveFilter(activeFilter === 'porte' ? null : 'porte')}
            >
              <div className="relative mt-2">
                <button 
            ...
            <FilterItem 
              ...
              icon={ShieldCheck} 
              active={activeFilter === 'natureza'} 
              onClick={() => setActiveFilter(activeFilter === 'natureza' ? null : 'natureza')}
            >
              <div className="relative mt-2">
                <div className="relative">
                  <input 
            ...
            <FilterItem 
              ...
              icon={Calendar} 
              active={activeFilter === 'abertura'} 
              onClick={() => setActiveFilter(activeFilter === 'abertura' ? null : 'abertura')}
            >
          ...
          {!showResults && !isLoading ? (
            ...
            >
              ...
              <div className="relative mb-16 text-center">
                <div className="absolute -left-24 top-0 hidden 2xl:block z-20">
                  <motion.div
                    initial={{ opacity: 0, scale: 0.8, x: 20, y: 10 }}
                    animate={{ opacity: 1, scale: 1, x: 0, y: 0 }}
                    transition={{ delay: 0.5, duration: 1.2, type: "spring", bounce: 0.4 }}
                  >
                    <svg width="180" height="120" viewBox="0 0 180 120" fill="none" className="text-emerald-500 absolute -top-10 -left-12 drop-shadow-md">
                      {/* Seta sinuosa com loop estilo desenho à mão */}
                      <motion.path 
                        d="M170 100 C 180 60, 100 0, 70 30 C 40 60, 90 80, 80 40 C 70 0, 20 20, 10 50" 
                        stroke="currentColor" 
                        strokeWidth="3.5" 
                        strokeLinecap="round" 
                      ...
                      />
                      {/* Ponta da seta apontando para a esquerda/baixo (para a sidebar) */}
                      <motion.path 
                        d="M10 50 L 5 40 M 10 50 L 25 45" 
                        stroke="currentColor" 
                        strokeWidth="3.5" 
                        strokeLinecap="round" 
                        strokeLinejoin="round"
                    ...
                    <motion.div 
                      ...
                      className="absolute top-[85px] left-[90px] bg-emerald-500 text-white px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest rotate-[-12deg] shadow-lg border border-emerald-400"
                    >
                      Filtre aqui!
                    </motion.div>
                  </motion.div>
                </div>
                <h2 className="text-4xl font-black text-brand-text mb-6 tracking-tighter uppercase leading-none">
              ...
              {/* Central Search Bar */}
              <div className="bg-brand-card rounded-[32px] shadow-[var(--brand-shadow)] border border-brand-border p-3 flex items-center gap-4 mb-20">
                <div className="flex-1 flex items-center gap-5 pl-8">
                  <Search className="w-6 h-6 text-brand-text-dim/40" />
                  <input 
                    type="text" 
                    placeholder="Descreva atividades ou nichos e deixe a IA te ajudar..." 
                    value={iaQuery}
                    onChange={(e) => setIaQuery(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && handleGenerate()}
                    className="w-full bg-transparent border-none outline-none text-base font-medium text-brand-text placeholder:text-brand-text-dim/50"
                  />
                </div>
                <button 
                  onClick={handleGenerate}
                  disabled={isLoading}
                  className="bg-[#2c524d] hover:bg-[#1f3a36] text-white px-10 py-5 rounded-[24px] font-black tracking-widest uppercase text-xs flex items-center gap-3 transition-all shadow-xl hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
                >
