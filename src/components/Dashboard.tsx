    ...
    >
      ...
      <AnimatePresence>
        {isNudgeVisible && (
          <motion.div 
            ...
            className="fixed bottom-8 right-8 z-[100] w-[380px] glass-card p-6 shadow-2xl border-brand-primary/20 overflow-hidden bg-brand-bg/80 backdrop-blur-2xl"
          >
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-brand-primary uppercase tracking-widest leading-none">Mineração Ativa</span>
                </div>
                <button 
                  onClick={() => setIsNudgeVisible(false)}
                  className="p-1.5 text-brand-text-dim hover:text-brand-text transition-colors rounded-lg hover:bg-brand-hover"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-black text-brand-text uppercase tracking-tighter leading-tight mb-2">
                    Pronto para <br />novos leads?
                  </h3>
                  <p className="text-brand-text-dim text-xs font-medium leading-relaxed">
                    Nossa IA está pronta para minerar os melhores contatos para o seu negócio. Comece sua próxima prospecção.
                  </p>
                </div>
                
                <button 
                  onClick={onStartSearch}
                  className="w-full group flex items-center justify-center gap-3 bg-brand-primary text-white py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-accent transition-all shadow-lg shadow-brand-primary/20"
                >
                  Encontrar agora
            ...
            {/* Subtle glow */}
            <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-32 h-32 bg-brand-primary/10 rounded-full blur-2xl pointer-events-none" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card p-8">
          ...
          <div className="h-[350px] w-full flex items-center justify-center bg-brand-bg/30 rounded-2xl border border-brand-border/50 relative overflow-hidden p-2">
            <div className="relative w-full h-full">
              <ComposableMap
                ...
                className="w-full h-full drop-shadow-[0_0_20px_rgba(0,0,0,0.4)] saturate-150"
              >
                <Geographies geography="https://raw.githubusercontent.com/fititnt/gis-dataset-brasil/master/uf/geojson/uf.json">
                  {({ geographies }) =>
                    geographies.map((geo) => {
                      const isHighlighted = ['SC', 'PR', 'RS', 'SP', 'MG'].includes(geo.properties.UF_05);
                      return (
                        <Geography
                          key={geo.rsmKey}
                          geography={geo}
                          fill={isHighlighted ? 'var(--brand-primary)' : 'var(--brand-bg)'}
                          ...
                          style={{
                            default: {
                              ...
                              outline: 'none',
                              filter: isHighlighted ? 'drop-shadow(0px 2px 4px rgba(0,0,0,0.5))' : 'none'
                            },
                            hover: {
              ...
              {/* Informative overlay boxes */}
              <div className="absolute bottom-4 right-4 bg-brand-card/90 backdrop-blur-md border border-brand-border p-4 rounded-xl shadow-2xl">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-2.5 h-2.5 rounded-full bg-brand-primary animate-pulse" />
                    <span className="text-[10px] font-black text-brand-text uppercase tracking-widest leading-none">SP: 45.200 Leads</span>
                  </div>
                  <div className="flex items-center gap-3 opacity-80">
                    <div className="w-2.5 h-2.5 rounded-full bg-brand-primary" />
                    <span className="text-[10px] font-black text-brand-text uppercase tracking-widest leading-none">SC: 34.500 Leads</span>
                  </div>
                  <div className="flex items-center gap-3 opacity-60">
                    <div className="w-2.5 h-2.5 rounded-full bg-brand-primary/60" />
                    <span className="text-[10px] font-black text-brand-text uppercase tracking-widest leading-none">PR: 25.100 Leads</span>
                  </div>
                  <div className="flex items-center gap-3 opacity-40">
                    <div className="w-2.5 h-2.5 rounded-full bg-brand-primary/40" />
                    <span className="text-[10px] font-black text-brand-text uppercase tracking-widest leading-none">RS: 16.450 Leads</span>
                  </div>
                </div>
              </div>
            </div>
